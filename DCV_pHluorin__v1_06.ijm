...
Modify
	...
	2018.02.09 - add batch detection
	2018.04.13 - add sub pixel localization (using center of mass) and extra filtering for smoothing
	2018.04.16 - add function to fix the roiManger
	2018.08.08 - adjust export to SynD
*/
...
function setDefaultParameters(arg){
	...
	call("ij.Prefs.set", "detectionPar.subPixel", false);
	call("ij.Prefs.set", "detectionPar.semiAuto", false);
...
function detectionParameters(){
	...
	subPixel = call("ij.Prefs.get", "detectionPar.subPixel", true);
	semiAuto = call("ij.Prefs.get", "detectionPar.semiAuto", true);
	...
	Dialog.addNumber("SNR (between 3-20)", snr);
	...
	batchAnalysis = Dialog.getCheckbox();
	...
	if(advOption){
		...
		if(bReset){
			...
			call("ij.Prefs.set", "detectionPar.subPixel", subPixel);
			call("ij.Prefs.set", "detectionPar.semiAuto", semiAuto);
		}
	}
}
...
var sCmds1 = newMenu("Start Analysis Menu Tool", newArray("Fusion detection", "Export to SynD", "Get NH4 pool", "-", "Options"));
...
var sCmds2 = newMenu("ROIs Interacion Menu Tool", newArray("Save ROI", "Measure ROIs", "Move through ROIs", "Fix ROI Manager", "-", "Options"));
...
function saveForSynD(title, marker, baseFrame, startF, endF){
	...
	if (marker == "pHluorin"){
		...
		run("Z Project...", "projection=[Max Intensity]");
		rename("MAX");
		imageCalculator("Subtract create", "MAX","firstFrame");
		selectWindow("firstFrame");
		close();
		selectWindow("MAX");
		close();
		selectWindow("Result of MAX");
		...
		run("Enhance Local Contrast (CLAHE)", "blocksize=150 histogram=256 maximum=2.5 mask=*None*");
		run("Enhance Contrast...", "saturated=0.3 normalize");
		run("16-bit");
		rename("morph");
		run("Merge Channels...", "c1=MAX c2=morph c3=MAX_Average create");
	} else {
...
function detectParticles(){
	...
	if(!bDetection){
		...
		if(batchAnalysis){
			...
			nFiles = workFile.length;
		}
		for(f=0; f<nFiles; f++){
			if(batchAnalysis){
				if(f>0){
					tempFile = workDir + workFile[f];
					if(endsWith(tempFile, "tif")){
						open(tempFile);
						orTitle = getTitle();
					}
				}
			...
			for(iter=0;iter<nIteration;iter++){
				...
				if(nSlices<1000){
					// Might be suboptimal but the reason is to clean a bit more the image
					showStatus("Run Top Hat filter - Particle detection v-"+majVer+"."+minVer);
					run("Morphological Filters (3D)", "operation=[White Top Hat] element=Square x-radius="+ROI_size+" y-radius="+ROI_size+" z-radius=1");
					rename("tempDiff");
					showStatus("Particle detection v-"+majVer+"."+minVer);
				...
				if(semiAuto){
					waitForUser("Manually place potential ROI");
				}else{
					particleDetection();
				}
	...
	*/
	...
	if(bRolling){
		...
		for(r=0;r<rollDiff.length;r++){
			...
			if(rollDiff[rIdx] > 0){
				/*
				//cleanLvl = vesicle[rIdx]/vesicle[rIdx-1];
				cleanArr = newArray(vesicle.length);
				Array.fill(cleanArr, cleanLvl);
				setBatchMode("exit and display");
				Plot.create("test", "Frames", "Intensity", vesicle);
				Plot.setColor("red");
				Plot.add("line", rollStd);
				Plot.setColor("blue");
				Plot.add("circles", rollDiff);
				Plot.setColor("magenta");
				Plot.add("line", cleanArr);
				Plot.setColor("black");
				Plot.setLimits(NaN,NaN,NaN,NaN);
				Plot.show();
				waitForUser("r = " + rIdx + ". SNR("+vesicle[rIdx]+"/"+vesicle[rIdx-1]+") = " + cleanLvl);
				close();
				*/
				// there is a candidate
				if(vesicle[rIdx] > cleanLvl){
				//if(cleSigma <= cleanLvl){
					tempSlice = rIdx + 1;
...
// workflow for getting the most out of the NH4 response
function NH4_pool(){
	orTitle = getTitle();
	detectionParameters();
	bpHluorin = getBoolean("Which fluorophore was used?", "pHluorin", "mCherry");
	if(bpHluorin){
		run("Duplicate...", "title=tempDiff duplicate");
		subtrackStack();
		...
		rename("MAX_Diff_" + orTitle);
		selectWindow("Diff_" + orTitle);
		run("Gaussian Blur...", "sigma=" + sigma + " stack");
		run("Morphological Filters (3D)", "operation=[White Top Hat] element=Ball x-radius=2 y-radius=2 z-radius=1");
	}
	rename("NH4_response");
	selectWindow("Diff_" + orTitle);
	close();
	selectWindow("NH4_response");
	nIter = getNumber("Number of iteration: ", 5);
	...
	setBatchMode("hide");
	setBatchMode(true);
	cleanupRoiManager(roiOver);
	setBatchMode("exit and display");
	selectWindow("NH4_response-1");
	close();

	
	// get the number of pixels and their intensity
	nRoi = roiManager("Count");
	roiValue = newArray(nRoi);
	selectWindow(orTitle);
	selectWindow("MAX_Diff_" + orTitle);
	for(r=0;r<nRoi;r++){
		roiManager("Select", r);
		getStatistics(roiArea, roiMean, roiMin, roiMax, roiStd, roiHist);
		roiValue[r] = roiMean;
	}
	newImage("tempValue", "32-bit black", 1 , nRoi, 1);
	for(h=0;h<nRoi;h++){
		setPixel(0,h,roiValue[h]);
	}
	List.setMeasurements;
	mode = List.getValue("Mode");
	selectWindow("tempValue");
	close();
	selectWindow("MAX_Diff_" + orTitle);
	close();
	adjustValue = newArray(nRoi);
	adjustCount = 0;
	for(r=0;r<nRoi;r++){
		adjustValue[r] = floor(roiValue[r]/mode);
		if(adjustValue[r]==0)
			adjustValue[r] = 1;
		adjustCount = adjustCount + adjustValue[r];
	}
	Array.print(roiValue);
	print("Mode: " + mode);
	print("Raw pool: " + nRoi);
	print("Adjust pool: " + adjustCount);
}
